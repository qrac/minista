 ...
 * @param {string} outputAssetPath
 * @returns {string}
 */
export function getBasedAssetUrl(base, outputHtmlPath, outputAssetPath) {
  if (base === "./") {
    return normalizePath(
      path.relative(path.dirname(outputHtmlPath), outputAssetPath)
    )
  }
  return normalizePath(base.replace(/\/$/, "") + "/" + outputAssetPath)
}
