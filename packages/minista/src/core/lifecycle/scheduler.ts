...
import type { FeatureId } from "../graph/index.js"
import type { MinistaFeature } from "./types.js"

export function scheduleFeatures(
  features: readonly MinistaFeature[],
  diagnostics: DiagnosticCollector,
): readonly MinistaFeature[] {
  const byId = new Map<FeatureId, MinistaFeature>()
  ...
  const providers = new Map<string, FeatureId[]>()
  ...
  const dependencies = new Map<FeatureId, Set<FeatureId>>()
  for (const feature of byId.values()) {
    ...
    for (const capability of feature.requires ?? []) {
      ...
      for (const provider of capabilityProviders) {
        if (provider !== feature.id) required.add(provider)
      }
    }
    dependencies.set(feature.id, required)
  }

  if (diagnostics.hasErrors()) return Object.freeze([])

  const result: MinistaFeature[] = []
