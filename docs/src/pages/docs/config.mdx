...
render／clientでpluginの名前や順序が変わるconfigはApp Buildで共有できません。この場合ministaはstructured diagnosticを記録し、同一processのcompatibility builderへfallbackします。CoreはViteのenvironment型やhookには依存しません。
