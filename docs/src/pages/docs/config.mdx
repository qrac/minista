...
通常のbuild／devはprogrammatic Vite adapterを使用します。programmatic configへ安全に変換できないVite CLI flagを指定した場合だけ外部CLIへfallbackします。Coreはどちらの経路でもViteのenvironment型やhookに依存しません。

### Asset inline error

pageからimportした画像をバンドルした際にビルドが失敗する場合があります。これは主にViteが小さい画像をclientではインライン判定したものの、Node.js向けrender environmentでは同じ形に変換されないことが原因です。
...
`pluginSsg`はrenderer出力に含まれるimage preloadを既定で除去します。残したい場合は`pluginSsg({ removeImagePreload: false })`を指定してください。`Head` APIで明示したpreloadは既定でも保持します。詳細は[pluginSsg](/docs/plugins/ssg)を参照してください。
