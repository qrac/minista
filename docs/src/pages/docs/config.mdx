...
ministaはrender environmentとclient environmentを1つのApp Build lifecycleで順に実行します。多くのVite設定はそのまま両environmentへ適用できます。

既存の`isSsrBuild`を使ったconfig関数もcompatibility adapterがenvironmentごとに評価します。Node.jsでpageをrenderする側とbrowser向けassetを生成する側で設定を分けたい場合に使用できます。
...
isSsrBuildを参照するconfigは、同名pluginでも設定を取り違えないよう、`MINISTA_VITE_APP_CONFIG_LEGACY_ENVIRONMENT`を記録して同一processのcompatibility builderへfallbackします。pluginの名前や順序も変わる場合は`MINISTA_VITE_APP_CONFIG_PLUGIN_MISMATCH`を記録します。新しいconfigではplugin内部のenvironment-aware hookで処理を分けることを推奨します。
