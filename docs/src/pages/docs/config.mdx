...
render／clientでpluginの名前や順序が変わるconfigはApp Buildで共有できません。この場合ministaは`MINISTA_VITE_APP_CONFIG_PLUGIN_MISMATCH`を記録し、同一processのcompatibility builderへfallbackします。新しいconfigでは両environmentのplugin構成を揃え、plugin内部でenvironmentごとの処理を分けることを推奨します。
...
`pluginBundle`を使って画像をバンドルした際にビルドが失敗する場合があります。これは主にViteが小さい画像をclientではインライン判定したものの、Node.js向けrender environmentでは同じ形に変換されないことが原因です。
