...
3 年後、Vite と完全に統合するための minista v4 を公開しました。
...
開発環境はビルドの手間を無くすために TypeScript を JavaScript + JSDoc に変更。キーカラーには OKLCH を採用。ドキュメントにライトテーマを追加しています。

新設計の minista と Rolldown ベースとなった Vite を併用することで、より高速で柔軟な静的サイト生成が可能になりました。
