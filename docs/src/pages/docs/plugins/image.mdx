...
画像参照を`analyze`、変換planとbinary Artifactを`generate`、確定URLを`compose` phaseで反映します。devではpage scope付き参照Artifactをserver lifetimeのsessionに保持するため、変更画像を参照するページだけを更新できます。
...
リモート画像のsource cache方針です。`"immutable"`はキャッシュを削除するまで同じURLのsourceを再取得しません。`{ maxAge }`はミリ秒単位の有効期間を指定し、期限後はETagまたはLast-Modifiedがあれば条件付きrequestで再検証します。`useCache: false`の場合は使用されません。
...
[sharp](https://www.npmjs.com/package/sharp)のResizeにフィット方法を指定します。
...
[sharp](https://www.npmjs.com/package/sharp)のResizeにトリミングやリサイズ時の重心位置を指定します。
...
[sharp](https://www.npmjs.com/package/sharp)のResizeに背景色を指定します。主に `fit` が `"contain"` の場合に有効となります。
...
画像処理に失敗した場合、buildは次のstable diagnostic codeを出力します。

- `MINISTA_IMAGE_DOWNLOAD_FAILED`: リモート画像の取得失敗
- `MINISTA_IMAGE_READ_FAILED`: local画像の欠落または読込失敗
- `MINISTA_IMAGE_METADATA_FAILED`: 画像サイズ／形式の解析失敗
- `MINISTA_IMAGE_TRANSFORM_FAILED`: Sharpによる変換失敗
- `MINISTA_IMAGE_CACHE_FAILED`: cacheの読込／書込失敗

local画像のdiagnosticにはproject相対locationが含まれます。リモートURLのquery parameterはdiagnostic messageへ出力しません。
