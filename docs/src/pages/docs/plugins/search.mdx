...
対象ページを共有Document／Page Graphへ投影し、ページ解析Artifactを`analyze`、検索JSONを`generate`、検索UIの相対階層属性を`compose` phaseで生成します。devのJSON endpointもbuildと同じSearch featureを使用します。
