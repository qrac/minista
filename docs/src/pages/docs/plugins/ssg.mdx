...
document Layoutでも既存の`Head`コンポーネントを併用できます。`Head`の属性とhead要素はLayoutに後から適用されます。`title`、charset、viewportが重複する場合は`Head`側が優先されます。どちらにもcharsetまたはviewportがない場合はMinistaが既定値を補います。出力時はcharsetをheadの先頭、viewportをその次へ配置します。
