...
- 型: `false | CompileOptions`
- デフォルト: `{ remarkPlugins: [], rehypePlugins: [] }`

MDX・Markdownの変換設定です。デフォルトで有効ですが、`@mdx-js/mdx`とprocessorは最初の`.md`または`.mdx`が読み込まれるまでロードされません。無効にする場合は`mdx: false`を指定します。`src`を省略している場合、無効化に合わせてdefaultの探索対象もJSX・TSXだけになります。
