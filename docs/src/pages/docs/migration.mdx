...
Island／Beautifyのoption型と`minista/client`のMD／MDX宣言の参照を修正しました。Archive／Beautifyの宣言に必要な型依存はministaと一緒にインストールされます。Reactに対応する`@types/react`／`@types/react-dom`をprojectへ追加してください。React 18／19とも`skipLibCheck:false`で公開型を利用できます。既存のoption名や設定値に変更はありません。
