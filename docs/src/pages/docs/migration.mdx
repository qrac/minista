...
公開plugin APIと基本的なVite configは継続して使用できます。内部buildは、Vite CLIをrender用とclient用に2回起動する方式から、render environmentとclient environmentを1つのApp Build lifecycleで管理する方式へ変更されました。
...
v4ではViteのSSRビルドと通常ビルドを連続で行っていました。v5ではrender environmentとclient environmentを1つのApp Build lifecycleで実行します。両方へ適用する設定はrootへ、片方だけに適用する設定はnamed environmentかpluginのenvironment-aware hookへ記述してください。
