...
v3 の Partial Hydration は v4 の Island に移行してください。コンポーネント側の書き方は `partial` 系の指定から `client:*` ディレクティブに変わります。

```tsx title="Before"
import { Counter } from "../components/counter"

export default function () {
  return <Counter partialHydration />
}
```
...
IntersectionObserver で表示時に実行していたものは `client:visible` に移行してください。
