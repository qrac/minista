...
v3 では任意のディレクトリの minista プロジェクトを動かせます。この機能を他のフレームワークの慣習に習って root オプションと命名しました。その関係でページ全体をラップする要素 `root.tsx (or .jsx)` が紛らわしい状態になってしまったため `global.tsx (or .jsx)` に変更しました。

ただ、引き続き `root.tsx (or .jsx)` でも動作はしますので修正は必須ではありません。
...
今まで通り SVG ファイルをインライン展開するためには、default export で読み込んでいた箇所を named export の読み込みに変更する必要があります。
...
スプライトアイコンの生成は専用コンポーネントによる処理に変更したため修正が必要です。v2 では出力先のパスを指定する必要があり、この仕様は違和感がありました。v3 の専用コンポーネントでは ID のみを渡せばよくなっています。
...
他の命名と異なり `MinistaLocation` のみスコープ名だったので `Location` に変更しました。v2 で`MinistaLocation` を参照していた部分はエラーが出ますので `Location` に変更してください。
