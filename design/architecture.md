...
production featureのdomain phaseはCore runnerへ接続済みです。全production compatibility featureのLifecycleRunnerは同じbuild sessionが所有するmutable Project Graph、Document Store、Artifact Store、Emitter、diagnostic collectorを直接操作し、scope付きphase traceを同じevent列へ追加します。同一output pageはfile identityから同じDocument instanceを再利用し、feature再実行時はそのfeature所有Artifactだけを置換して他featureのhandoffを保持します。同一Assetのconsumer、Island／Imageのpage／generated asset関係はGraph登録時にmergeします。output APIは共有Emitterを使いながら呼出しごとの差分だけをVite hookへ返します。build session disposalはworkspace参照とArtifactを解放します。SSG renderは同じArtifact Store／diagnostic collector／traceを使いますが、Route／Page Graphの正本はproduction render snapshotとして別に保持します。dev server adapterもserver lifetimeのsessionを作成してclose時に破棄し、Comment／Svg／Sprite／Imageの `transformIndexHtml()` は同じDocument／Graph／Artifact／trace workspaceを共有します。Artifactの明示的なpage scopeにより、SpriteとImageは更新対象外ページの参照Artifactを保持しながら入力ページと集約生成物だけを置換します。Island／SSGなどのdev処理にはfeature別cacheと手続き的なHTML処理が残ります。
