...
| dev feature | server lifetimeのsession、入力ページ限定Document phase、page scope付きArtifact更新 | Search JSON生成など一部のadapter固有処理は個別経路に残る |
...
production featureのdomain phaseはCore runnerへ接続済みです。全production compatibility featureのLifecycleRunnerは同じbuild sessionが所有するmutable Project Graph、Document Store、Artifact Store、Emitter、diagnostic collectorを直接操作し、scope付きphase traceを同じevent列へ追加します。同一output pageはfile identityから同じDocument instanceを再利用し、feature再実行時はそのfeature所有Artifactだけを置換して他featureのhandoffを保持します。同一Assetのconsumer、Island／Imageのpage／generated asset関係はGraph登録時にmergeします。output APIは共有Emitterを使いながら呼出しごとの差分だけをVite hookへ返します。build session disposalはworkspace参照とArtifactを解放します。SSG renderは同じArtifact Store／diagnostic collector／traceを使いますが、Route／Page Graphの正本はproduction render snapshotとして別に保持します。dev server adapterもserver lifetimeのsessionを作成してclose時に破棄し、Comment／Svg／Sprite／Image／Islandの `transformIndexHtml()` は同じDocument／Graph／Artifact／trace workspaceを共有します。incremental phaseへ渡すDocumentは現在の入力ページへ限定し、page scope付きArtifactから全ページの集約生成物を再構成します。SSGのdev renderもRoute／Page Graph、共有Artifact Store、diagnostic collector、scope付きtraceをCore render phaseへ渡し、render cacheはrenderer portの内側で維持します。Search JSON生成など一部のdev adapter処理は個別経路に残ります。
