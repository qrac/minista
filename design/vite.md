...
| `transformIndexHtml()` hook context | Vite 8.2.1ではenvironmentを公開せず、serverもoptional | SSG／Sprite／Image／Svg／Island devは登録server identityをadapterで解決。他featureは長寿命Document lifecycleへの移行までconfig stateを明示管理 |
...
- compatibility graph／outputs: hook結果をsession Graph／Emitterへ決定的に集約
