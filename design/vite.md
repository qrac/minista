...
| `transformIndexHtml()` hook context | Vite 8.2.1ではenvironmentを公開せず、serverもoptional | Sprite／Image devは登録server identityをadapterで解決。他featureは長寿命Document lifecycleへの移行までconfig stateを明示管理 |
