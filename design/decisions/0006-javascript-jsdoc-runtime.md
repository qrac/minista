...
`npm run test:public-types`はpackしたpackageをリポジトリ外の空consumerへインストールし、React 19で`strict:true`／`skipLibCheck:false`の検査を実行します。公開plugin、component、公開page型、client宣言と不正値の拒否を対象とします。packと隔離consumerへの依存インストールを伴うため、公開宣言・配布依存に関係する変更時だけ明示実行します。通常CIでは実行しません（[検証構成](../testing.md)）。上流型との比較は通常の`typecheck`で実行し、配布型の厳格検査と分けます。
