...
- programmatic／外部CLI fallbackを縮退し、programmatic buildの失敗snapshotとdevのVite由来errorをstructured diagnosticへ正規化する
